...
/* The table itself */
struct xt_table_info { // 4 + 4 + 4 + 4*5 + 4*5 + 4 + 8 = 64
	/* Size per table */
	unsigned int size; // 4
	/* Number of entries: FIXME. --RR */
	unsigned int number; // 4
	/* Initial number of entries. Needed for module usage count */
	unsigned int initial_entries; // 4

	/* Entry points and underflows */
	unsigned int hook_entry[NF_INET_NUMHOOKS]; //  4*5
	unsigned int underflow[NF_INET_NUMHOOKS]; // 4*5
	...
	unsigned char entries[] __aligned(8);  // 柔性数组 编译器来说，此时长度为0的数组并不占用空间
};
