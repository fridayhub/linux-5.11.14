				...
				unsigned int *size)
{
	...
	t = *dstptr;
	memcpy(t, ct, sizeof(*ct)); // 拷贝数据到新分配的 newinfo的 entries
	if (target->compat_from_user)
		target->compat_from_user(t->data, ct->data);
	else
		memcpy(t->data, ct->data, tsize - sizeof(*ct));
	pad = XT_ALIGN(target->targetsize) - target->targetsize; // 按照 _u64 8字节对齐(起始地址都将是8的倍数)
	if (pad > 0)
...
struct xt_table_info *xt_alloc_table_info(unsigned int size)
{
	...
	size_t sz = sizeof(*info) + size; // sizeof(*info) = 0x40 = 64,  指针 info 指向的对象的大小，而不是指针本身的大小
	// sz = 0x40 + 0xfb6 = FF6 = 4086
