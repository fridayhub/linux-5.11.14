			    ...
			    struct xt_table_info *newinfo, unsigned char *base)
{
	...
	*dstptr += sizeof(struct ipt_entry); // 指针跳过 ipt_entry头部
	*size += sizeof(struct ipt_entry) - sizeof(struct compat_ipt_entry);

	xt_ematch_foreach(ematch, e)
		xt_compat_match_from_user(ematch, dstptr, size); // 进入漏洞

	de->target_offset = e->target_offset - (origsize - *size);
	t = compat_ipt_get_target(e);
	// t 指向用户数据的最后一项：xt_entry_target
	// 
	xt_compat_target_from_user(t, dstptr, size); // 进入漏洞
		       ...
		       const struct compat_ipt_replace *compatr)
{
	...
	info = *pinfo;
	entry0 = *pentry0;  // 与上一个函数的loc_cpu_entry指向同一个位置
	size = compatr->size;
	...
	ret = -ENOMEM;
	newinfo = xt_alloc_table_info(size); // size = 0xfbe = 4030
	if (!newinfo)
		goto out_unlock;
	...
	size = compatr->size;
	/* 展开后
	for ((iter0) = (typeof(iter0))(entry0); \
         (iter0) < (typeof(iter0))((char *)(entry0) + (esize)); \
         (iter0) = (typeof(iter0))((char *)(iter0) + (iter0)->next_offset))

	所以对于第一个循环来讲：
	iter0 指向 entry0 也就是指向 pentry0
	pos 指向新分配的 newinfo的 entries
	size 就是tmp的size
	entry1 指向新分配的 newinfo的 entries
	*/
	xt_entry_foreach(iter0, entry0, compatr->size)
...
{
	// user model： setsockopt(socket_fd, SOL_IP, IPT_SO_SET_REPLACE, &data, sizeof(data)
	// arg = &data  len = sizeof(data) 对应用户模式的 data 和 长度
	int ret;
	...
	if (copy_from_sockptr(&tmp, arg, sizeof(tmp)) != 0) // 取出用户数据的 compat_ipt_replace 层
		return -EFAULT;
	...
	newinfo = xt_alloc_table_info(tmp.size); // tmp.size = 0xFB6 = 4022
	if (!newinfo)
		return -ENOMEM;

	loc_cpu_entry = newinfo->entries;
	if (copy_from_sockptr_offset(loc_cpu_entry, arg, sizeof(tmp),
			tmp.size) != 0) {  // 拷贝用户data剩余的数据
		ret = -EFAULT;
		goto free_newinfo;
	}

	ret = translate_compat_table(net, &newinfo, &loc_cpu_entry, &tmp);  // 进入
	if (ret != 0)
		goto free_newinfo;
...
do_ipt_set_ctl(struct sock *sk, int cmd, sockptr_t arg, unsigned int len)
{
	...
	switch (cmd) {
	case IPT_SO_SET_REPLACE:  // 漏洞触发case
#ifdef CONFIG_COMPAT
		if (in_compat_syscall())
